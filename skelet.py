from lib_skelet import pics_list, loader,fullcolor_raw0, fullcolor_lum1, grayscale_red2, grayscale_green3, grayscale_blue4, display_image
...
def main():
    ...
    # shows both titles, timing their duration
    ...
    nicely_tabulated = loadfile(pictures.items()) # passes a list of key-value tuples from previous dict to tabulate

    # inputs user for file to be processed
    while True:

        file_choice = input(f"{nicely_tabulated}\n\nfile number: ") # passes table for user to choose the file
    ...
    while True:
        ...
        file_ = file.split(".")[0] # starts editing file name to be used for file naming
        display_option = input(f"{display_options()}\n\nprocessing type: ") # shows modes of display of the picture while capturing input
